...
// MVID: 12B286CD-3907-4064-8175-71FE919069D8
// Assembly location: C:\Users\Dicku\Desktop\Photon3Unity3D.dll
...
namespace Client.Photon
{
    public class Protocol
    {
        ...
        static void SerializeOperationRequest(MemoryStream memStream, OperationRequest serObject, bool setType)
        {
            Protocol.SerializeOperationRequest(memStream, serObject.OperationCode, serObject.Parameters, setType);
        }

        public static void SerializeOperationRequest(MemoryStream memStream, byte operationCode, Dictionary<byte, object> parameters, bool setType)
        {
            if (setType)
        ...
        public static void SerializeOperationResponse(MemoryStream memStream, OperationResponse serObject, bool setType)
        {
            if (setType)
        ...
        public static OperationResponse DeserializeOperationResponse(MemoryStream memoryStream)
        {
        ...
        public static void SerializeEventData(MemoryStream memStream, EventData serObject, bool setType)
        {
            if (setType)
        ...
        public static EventData DeserializeEventData(MemoryStream din)
        {
            ...
            else
            {
                ...
                switch (Type.GetTypeCode(type))
                {
                    ...
                    case TypeCode.String:
                        ...
                        break;
                    default:
                        if (serObject is ExitGames.Client.Photon.Hashtable)
                        {
                            Protocol.SerializeHashTable(dout, (ExitGames.Client.Photon.Hashtable)serObject, setType);
                            break;
                        }
                        if (type.IsArray)
                        {
            ...
            byte[] buffer = new byte[5]
      {
        ...
        private static void SerializeHashTable(MemoryStream dout, ExitGames.Client.Photon.Hashtable serObject, bool setType)
        {
            if (setType)
        ...
        private static void SerializeDictionaryHeader(MemoryStream writer, object dict, out bool setKeyType, out bool setValueType)
        {
            ...
            {
                GpType gpType = (GpType)Protocol.GetCodeOfType(genericArguments[0]);
                switch (gpType)
                {
                    case GpType.Unknown:
            ...
            {
                GpType gpType = (GpType)Protocol.GetCodeOfType(genericArguments[1]);
        ...
        private static object Deserialize(MemoryStream din, byte type)
        {
            ...
            else
            {
                switch (num)
                {
                    ...
                    case (byte)111:
                        return (object)(bool)(Protocol.DeserializeBoolean(din) ? true : false);
                    case (byte)112:
        ...
        private static ExitGames.Client.Photon.Hashtable DeserializeHashTable(MemoryStream din)
        {
            int x = (int)Protocol.DeserializeShort(din);
            ExitGames.Client.Photon.Hashtable hashtable = new ExitGames.Client.Photon.Hashtable(x);
